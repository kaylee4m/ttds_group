...
class PostingElement:
    ...
    def add_pos(self, pos):
        ...
        self.num_pos += 1
        if settings['cfg']['RECORD_POSITION']:
...
def load_pl_group_by_term(term):
    ...
    if os.path.exists(path):
        ...
        with open(path, 'rb') as dictfile:
            pl_group = pickle.load(dictfile)
        # lazy decoding, so dont decode here
...
def get_posting_list(term: str) -> PostingList:
    ...
    if type(posting_list) is not PostingList:
        pl_group[term] = PostingList.decode(term, posting_list)
        posting_list = pl_group[term]
    return posting_list
